...
/usr/bin/cp /tmp/monit-$_MONIT_VERSION/src/net/net.c /tmp/monit-$_MONIT_VERSION/src/net/net.c.original
/usr/bin/cp $_CYGMONIT_DEV_VERSION/Monits/$_MONIT_VERSION/src/net.c.patch /tmp/monit-$_MONIT_VERSION/src/net
/usr/bin/patch -p1 /tmp/monit-$_MONIT_VERSION/src/net/net.c /tmp/monit-$_MONIT_VERSION/src/net/net.c.patch

/usr/bin/cp /tmp/monit-$_MONIT_VERSION/src/net/Link.c /tmp/monit-$_MONIT_VERSION/src/net/Link.c.original
/usr/bin/cp $_CYGMONIT_DEV_VERSION/Monits/$_MONIT_VERSION/src/Link.c.patch /tmp/monit-$_MONIT_VERSION/src/net
/usr/bin/mkdir -p /tmp/monit-$_MONIT_VERSION/src/net/os/cygwin
/usr/bin/cp /tmp/monit-$_MONIT_VERSION/src/net/os/linux/Link.inc /tmp/monit-$_MONIT_VERSION/src/net/os/cygwin/Link.inc
/usr/bin/patch -p1 /tmp/monit-$_MONIT_VERSION/src/net/Link.c /tmp/monit-$_MONIT_VERSION/src/net/Link.c.patch
...
# There is no PAM for Cygwin (/usr/bin/autoupdate && ./bootstrap && autoreconf -i -f && automake -acf && ./configure --without-pam && make clean && make # --without-ssl #--without-ipv6)
./bootstrap
./configure --without-pam 
#--without-ssl  --without-ipv6 --without-largefiles --without-zlib

make clean && make
/usr/bin/cp /tmp/monit-$_MONIT_VERSION/monit.exe $_CYGMONIT_DEV_VERSION/Monits/$_MONIT_VERSION/exe/monit.exe 
...
/tmp/monit-$_MONIT_VERSION/monit --version